...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (action === "getInstructors") {
      var res = await notion("/databases/" + DB_INSTRUCTORS + "/query", "POST", {
        filter: { property: "狀態", select: { equals: "啟用" } }
      });
      result = res.results.map(function(p) {
        return {
          id: p.id,
          name: getTitle(p.properties["名稱"]),
          email: p.properties["Email"] ? p.properties["Email"].email || "" : "",
      ...
      var res = await notion("/databases/" + DB_COURSES + "/query", "POST", {
      ...
      result = res.results.map(function(p) {
        return {
          id: p.id,
          name: getTitle(p.properties["名稱"]),
          date: p.properties["日期"] && p.properties["日期"].date ? p.properties["日期"].date.start || "" : "",
          time: getSelect(p.properties["時段"]),
          location: getText(p.properties["地點"]),
          quota: p.properties["名額"] ? p.properties["名額"].number || 0 : 0
        };
      });
    }
      ...
      var res = await notion("/databases/" + DB_ENROLLMENTS + "/query", "POST", {});
      result = res.results.map(function(p) {
        return {
          id: p.id,
          instructorEmail: getText(p.properties["教官Email"]),
          courseName: getText(p.properties["課程名稱"]),
          enrollTime: p.properties["選課時間"] && p.properties["選課時間"].date ? p.properties["選課時間"].date.start || "" : ""
        };
      });
    }
      ...
      var res = await notion("/databases/" + DB_NOTIFICATIONS + "/query", "POST", {
      ...
      result = res.results.map(function(p) {
        return {
          ...
          time: p.properties["時間"] && p.properties["時間"].date ? p.properties["時間"].date.start || "" : ""
        };
      });
    }
